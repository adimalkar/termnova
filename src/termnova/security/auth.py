"""Verified request principals for local, API-key, browser-session, and OIDC access."""
...
AuthMethod = Literal["local", "api_key", "browser_session", "oidc"]
...
def is_valid_browser_session(
    token: str | None,
    settings: Settings,
    *,
    now: int | None = None,
) -> bool:
    """Verify a session token's version, expiry, and signature only.

    Deliberately independent of REQUIRE_AUTH so principal resolution cannot be
    short-circuited into trusting an unverified cookie.
    """
    ...
    expected_digest = hmac.new(signing_key.encode(), payload.encode(), hashlib.sha256).digest()
...
async def authenticate_credentials(
    ...
    *,
    ...
    if mode == "api_key":
        configured = _api_key(settings)
        method: AuthMethod
        if api_key is not None:
            if not configured or not hmac.compare_digest(api_key, configured):
                raise AuthenticationFailedError("Invalid or missing service API key")
            method = "api_key"
        elif is_valid_browser_session(browser_session, settings):
            # Browsers exchange the service key for a signed same-origin cookie
            # (see routes/auth.py); it represents the same service identity.
...
async def authenticate_websocket(websocket: WebSocket, settings: Settings) -> RequestPrincipal:
    ...
    return await authenticate_credentials(
        ...
        browser_session=websocket.cookies.get(BROWSER_SESSION_COOKIE),
        oidc_verifier=verifier,
    )
...
def verify_api_key(x_api_key: str | None = Header(default=None)) -> str:
    ...
    configured = _api_key(settings)
    if not x_api_key or not configured or not hmac.compare_digest(x_api_key, configured):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
